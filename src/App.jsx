...
function Header({ services, go }) {
  ...
  const navigate = (path) => { setMobileOpen(false); setServicesOpen(false); go(path); };
  return <header><a className="brand" href="/" onClick={(event) => { event.preventDefault(); navigate('/'); }}><img src="/alessandro-enterprises-logo.png" alt="" /><span>ALESSANDRO</span></a><button className="mobile-nav-toggle" aria-label="Open navigation menu" aria-expanded={mobileOpen} onClick={() => setMobileOpen(!mobileOpen)}>Menu</button><nav className={mobileOpen ? 'mobile-open' : ''}>
    <button onClick={() => navigate('/about')}>About</button><button onClick={() => navigate('/articles')}>Articles</button><button onClick={() => navigate('/events')}>Events</button><button onClick={() => navigate('/moments')}>Moments</button>
    <div className="services-menu"><button onClick={() => setServicesOpen(!servicesOpen)}>Services ▾</button>{servicesOpen && <div>{services.map((item) => <button key={item.id} onClick={() => navigate(`/service/${item.id}`)}>{item.name}</button>)}</div>}</div>
